...
import com.hcl.common.permissioncenter.api.IPermissionsResourceService;
import com.hcl.common.permissioncenter.api.PermissionCenterAtomServiceRegistry;
...
import com.hcl.common.permissioncenter.dao.IPermissionsResourceRepository;
...
public class PermissionsResourceServiceImpl extends BaseServiceImpl<TPermissionsResourceEntity, Long> implements IPermissionsResourceService {
    ...
    public List<SysPermissionTreeDto> treeByUidAndResType(Long uId, String resType) {
        ...
        List<TPermissionsUserRoleRelEntity> userRoleRelEntityList = PermissionCenterAtomServiceRegistry.getInstance().iPermissionsUserRoleRelService.findAllByUid(uId);
        List<Long> roleIds = EntityHelper.toIds(userRoleRelEntityList);
        if (roleIds == null || roleIds.isEmpty()){
            logger.info("listByUidAndResType not rel role");
            return treeDtoList;
        }
        List<TPermissionsResRoleRelEntity> resRoleRelEntityList = PermissionCenterAtomServiceRegistry.getInstance().iPermissionsResRoleRelService.findAllByRoleIdIn(roleIds);
